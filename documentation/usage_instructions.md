...
Each page also carries Open Graph tags, which are **not** an SEO feature —
Google ignores them. They control the preview card when somebody pastes the
link into Slack, LinkedIn, Bluesky or an email. One shared card for the whole
site, `images/og-card.png`: the logo centred on white at 1200x630, the size
those services expect. Title and description come from the page's own, so
there is nothing extra to keep in step.

Two things to change if the site ever moves to a custom domain: `og:url` and
`og:image` on all six pages, and the `Sitemap:` line in `robots.txt`. They are
the only absolute urls in the repository.
